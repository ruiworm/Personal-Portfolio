...
export const ProjectDetailPage: React.FC<ProjectDetailPageProps> = ({
  ...
  const prevLoc = prevProject ? getProjectLocalized(prevProject, lang) : null;
  const nextLoc = nextProject ? getProjectLocalized(nextProject, lang) : null;
  ...
  return (
    <div className="min-h-screen pt-20 pb-20 px-5 sm:px-6 max-w-4xl mx-auto relative z-10 selection:bg-cyan-500/30">
        ...
        >
          {copiedLink ? (
            <>
              ...
              <span className="text-emerald-400">{t('projectDetail.copied')}</span>
            </>
          ) : (
            <>
              <Share2 className="w-3.5 h-3.5" />
              <span>{t('projectDetail.share')}</span>
            </>
          )}
        </button>
      </motion.div>
        ...
        <div className="flex flex-wrap items-center gap-3 pt-1 border-t border-zinc-900/80">
          {project.link && project.link !== '#' && (
            ...
            >
              ...
              <span className="font-bold">{t('projectDetail.liveDemo')}</span>
          ...
          {project.github && (
            ...
            >
              ...
              <span className="font-medium">{t('projectDetail.github')}</span>
            </a>
          )}
          <div className="hidden sm:flex items-center gap-1.5 text-[11px] font-mono text-emerald-400/90 bg-emerald-950/30 border border-emerald-800/40 px-3 py-2 rounded-xl ml-auto">
            ...
            <span>{t('projectDetail.onlineBadge')}</span>
          </div>
        </div>
      </motion.header>
        ...
        <div className="px-4 py-2.5 bg-zinc-900/60 border-b border-zinc-800/80 flex items-center justify-between text-[11px] font-mono text-cyan-400/90 tracking-wider">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-zinc-200">
              {isDermaScan ? t('projectDetail.stationLabel') : `SYS.NODE_${project.id.toUpperCase()}`}
            </span>
          </div>
            ...
            {isDermaScan ? (
              <>
                <div className="flex items-center justify-between text-xs">
                  <span className="font-mono text-zinc-400">{t('projectDetail.modelLabel')}</span>
                  <span className="font-mono font-bold text-cyan-300">ConvNeXt-CBAM + Grad-CAM</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="font-mono text-zinc-400">{t('projectDetail.abcdeLabel')}</span>
                  <span className="font-mono font-bold text-emerald-400">
                    {lang === 'en' ? '5-D Morphological Scoring' : '5维形态学自动量化 (Asymmetry, Border...)'}
                  </span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="font-mono text-zinc-400">{t('projectDetail.presetLabel')}</span>
                  <span className="font-mono text-zinc-300">
                    {lang === 'en' ? '5 Follow-up Records · 6 Clinical Articles' : '5 份真实随访病案 · 6 篇专科审校图文百科'}
                  </span>
                </div>
                <div className="pt-2 border-t border-zinc-800/80 flex items-center justify-between">
                  <span className="text-[11px] font-mono text-zinc-500">
                    {lang === 'en' ? 'Status: Global Edge CDN' : '部署状态: 全球加速 CDN'}
                  </span>
                  <a
                    href={project.link}
                    target="_blank"
                  ...
                  >
                    <span>{t('projectDetail.openLive')}</span>
              ...
              <>
                <div className="flex items-center justify-between text-xs">
                  <span className="font-mono text-zinc-400">
                    {lang === 'en' ? 'System Architecture:' : '系统运行架构:'}
                  </span>
                  <span className="font-mono font-bold text-cyan-300">{project.category || 'SYSTEMS'}</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="font-mono text-zinc-400">
                    {lang === 'en' ? 'Benchmark Metric:' : '性能基准指标:'}
                  </span>
                  <span className="font-mono font-bold text-emerald-400">
                    {loc.specs && loc.specs[0] ? `${loc.specs[0].label}: ${loc.specs[0].value}` : 'N/A'}
                  </span>
                </div>
                <div className="pt-2 border-t border-zinc-800/80 flex items-center justify-between">
                  <span className="text-[11px] font-mono text-zinc-500">
                    {lang === 'en' ? 'System Status: Active' : '项目可用状态: 正常运行'}
                  </span>
                  {project.link && project.link !== '#' && (
                    <a
                      href={project.link}
                      ...
                      className="text-xs font-mono font-bold text-cyan-400 hover:text-cyan-300 flex items-center gap-1 transition-colors"
                    >
                      <span>{lang === 'en' ? 'Launch System' : '打开线上系统'}</span>
                      <ExternalLink size={12} />
                    </a>
                  )}
                </div>
      ...
      >
        ...
        {loc.specs && loc.specs.length > 0 && (
          <section className="space-y-3">
            <h3 className="text-xs font-mono tracking-[0.25em] text-cyan-400 uppercase flex items-center gap-2">
              ...
              <span>{t('projectDetail.secSpecs')}</span>
            </h3>
        ...
        {loc.features && loc.features.length > 0 && (
          <section className="space-y-3">
            <h3 className="text-xs font-mono tracking-[0.25em] text-cyan-400 uppercase flex items-center gap-2">
              ...
              <span>{t('projectDetail.secFeatures')}</span>
            </h3>
        ...
        {loc.architecture && loc.architecture.length > 0 && (
          <section className="space-y-3">
            <h3 className="text-xs font-mono tracking-[0.25em] text-cyan-400 uppercase flex items-center gap-2">
              <Cpu className="w-3.5 h-3.5" />
              <span>{t('projectDetail.secArchitecture')}</span>
            </h3>

            <div className="flex flex-wrap gap-2.5 font-mono text-xs">
              {loc.architecture.map((node, i) => (
                <div
                  key={node}
        ...
        {loc.challenges && (
          <section className="space-y-3">
            <h3 className="text-xs font-mono tracking-[0.25em] text-cyan-400 uppercase flex items-center gap-2">
              ...
              <span>{t('projectDetail.secChallenges')}</span>
            </h3>
        ...
        {nextProject && (
          ...
          >
            <div className="flex items-center justify-end gap-1.5 text-[10px] font-mono text-zinc-500 mb-1">
              <span>{t('projectDetail.next')}</span>
              <ChevronRight className="w-3 h-3 group-hover:translate-x-1 transition-transform" />
            </div>
            <div className="text-xs sm:text-sm font-semibold text-zinc-200 group-hover:text-cyan-300 transition-colors line-clamp-1">
              {nextLoc?.title || nextProject.title}
            </div>
          </button>
        )}
      </div>
